# Kevin Vincent (https://github.com/kevinvincent/ha-wyzesense)
...
class Packet(object):
    ...
    def Parse(cls, s):
        ...
        if cs_remote != cs_local:
            log.error("Invalid packet: %s", bytes_to_hex(s))
            log.error("Mismatched checksum, remote=%04X, local=%04X",
                      cs_remote, cs_local)
            return None
...
class SensorEvent(object):
    ...
    def __str__(self):
        s = "[%s][%s]" % (self.Timestamp.strftime(
            "%Y-%m-%d %H:%M:%S"), self.MAC)
        if self.Type == 'state':
            s += "StateEvent: sensor_type=%s, state=%s, battery=%d, signal=%d" % self.Data
        else:
            s += "RawEvent: type=%s, data=%s" % (
                self.Type, bytes_to_hex(self.Data))
        return s
...
class Dongle(object):
    ...
    def _OnSensorAlarm(self, pkt):
        ...
        timestamp, event_type, sensor_mac = struct.unpack_from(
            ">QB8s", pkt.Payload)
        ...
        if event_type == 0xA2:
            ...
            e = SensorEvent(sensor_mac, timestamp, "state",
                            (sensor_type, sensor_state, alarm_data[2], alarm_data[8]))
        else:
            e = SensorEvent(sensor_mac, timestamp, "raw_%02X" %
                            event_type, alarm_data)
    ...
    def _GetSensors(self):
        ...
        if count > 0:
            log.debug(
                "%d sensors reported, waiting for each one to report...", count)

            def cmd_handler(pkt, e):
                assert len(pkt.Payload) == 8
                mac = pkt.Payload.decode('ascii')
                log.debug("Sensor %d/%d, MAC:%s",
            ...
            self._DoCommand(Packet.GetSensorList(count),
                            cmd_handler, timeout=self._CMD_TIMEOUT * count)
        else:
    ...
    def Scan(self, timeout=60):
        ...
        def scan_handler(pkt):
            ...
            ctx.result = (pkt.Payload[1:9].decode(
                'ascii'), pkt.Payload[9], pkt.Payload[10])
            ctx.evt.set()
        ...
        try:
            ...
            if ctx.evt.wait(timeout):
                ...
                log.debug(
                    "Sensor found: mac=[%s], type=%d, version=%d", s_mac, s_type, s_ver)
    ...
    def Delete(self, mac):
        ...
        assert ack_mac == mac, "CmdDelSensor: MAC mismatch, requested:%s, returned:%s" % (
            mac, ack_mac)
